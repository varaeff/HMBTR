# ratings/russia-hmb-rating Specification

## Purpose

Defines a Russia HMB rating workflow that calculates official per-tournament
nomination points, stores them separately from Elo ratings, and exposes yearly
...
- **THEN** the system shows a centered Russia HMB rating calculation action below the top-three winners
...
- **THEN** the system shows a centered Russia HMB rating action below the top-three winners
...
- **WHEN** an administrator or secretary activates the Russia HMB rating calculation action
- **THEN** the system opens a modal with a top-right close action
- **AND** the modal shows a tournament coefficient label and a coefficient selector in one row
- **AND** the selector offers `1`, `2`, and `4`
- **AND** coefficient `1` is selected by default
- **AND** the modal shows a calculation action below the selector
...
- **AND** the losing fighter did not lose technically
...
#### Scenario: Technical losses do not award loss points
- **WHEN** a fighter loses a nomination fight technically by card, three warnings, or withdrawal
- **THEN** the losing fighter receives no Qc loss point for that fight
- **AND** persisted technical score shape or persisted technical round rows do not make the fighter eligible for Qc loss points
...
#### Scenario: Unexcused no-show penalties are counted
- **WHEN** a nomination participant has an active no-show withdrawal in the nomination
- **AND** the no-show is not marked as excused
- **THEN** the fighter receives `10` Qm penalty points for the no-show

#### Scenario: Excused no-show penalties are ignored
- **WHEN** a nomination participant has an active no-show withdrawal in the nomination
- **AND** the no-show is marked as excused
- **THEN** the fighter receives no Qm penalty points for that no-show
...
- **THEN** the modal title identifies the result list as Russia HMB rating
- **AND** the modal shows the saved nomination coefficient below the title
...
- **AND** the bottom action closes the modal

#### Scenario: Saved result action opens result list
- **WHEN** any user activates the Russia HMB rating action for a nomination with saved Russia HMB rating
...
- **THEN** the system shows a Russia HMB rating navigation action after the Elo rating navigation action
...
- **AND** the Elo fights column is replaced by a tournaments column that counts tournaments contributing to the total
...
- **THEN** the system does not show the Elo rating navigation action
...
- **THEN** the system shows the existing Elo rating navigation action as Elo rating
...
- **THEN** the page title identifies the page as Elo rating
...
- **THEN** the completed tournaments section includes a Russia HMB rating column for each nomination entry
...
- **THEN** the fighter profile shows a Russia HMB rating section after the fights/wins section
...
- **THEN** the existing Elo rating block is visible as Elo ratings
