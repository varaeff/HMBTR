...
export const evaluateSubmittedRawFightScoreForPersistence = (
  ...
  if (roundScores.length < rules.rounds) {
    throw new BadRequestException(`Expected at least ${rules.rounds} round scores`);
  }
  if (
    roundScores.some(
      (round) =>
    ...
    throw new BadRequestException('Scores must be non-negative 32-bit integers');
  }
  ...
  if (competitor1Total > MAX_SCORE || competitor2Total > MAX_SCORE) {
    throw new BadRequestException('Aggregate score exceeds the 32-bit integer limit');
  }
