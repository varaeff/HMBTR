...
export const groupFighters = (fighters: Fighter[]): Fighter[][] => {
  const total = fighters.length

  if (total === 0) return []
  if (total < 4) return [[...fighters]]
  ...
  const groups: Fighter[][] = Array.from({ length: groupCount }, () => [])

  // 3. Группируем по "весу" (клуб или город)
  const clusters: Record<string, Fighter[]> = {}
  shuffled.forEach((f) => {
