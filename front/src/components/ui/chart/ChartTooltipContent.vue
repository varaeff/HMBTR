...
type ChartTooltipPayloadValue = string | number | Date | null | undefined
...
const props = withDefaults(defineProps<{
  ...
  payload?: Record<string, ChartTooltipPayloadValue>
  config?: ChartConfig
...
const payload = computed(() => {
  return Object.entries(props.payload).map(([key, value]) => {
    ...
    const indicatorColor = props.config[key]?.color ?? String(props.payload.fill ?? "")
