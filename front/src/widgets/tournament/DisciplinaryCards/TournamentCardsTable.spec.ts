    ...
    resources: {
      en: {
        translation: {
          ...
          disciplinaryCardsActivate: 'Make active',
...
describe('TournamentCardsTable', () => {
  ...
  it('hides edit action for inactive yellow cards closed by an automatic red card', async () => {
    const pinia = createPinia()
    setActivePinia(pinia)
    const instance = await createI18n()
    ...
    expect(wrapper.text()).toContain('closed by red')
  ...
  it('does not render card status hints inside the card table', async () => {
    const pinia = createPinia()
    setActivePinia(pinia)
    const instance = await createI18n()
    ...
    expect(wrapper.find('[aria-hidden="true"]').attributes('title')).toBeUndefined()

    wrapper.unmount()
  })

  it('allows editing card type and expiration while keeping issue date read-only', async () => {
    const pinia = createPinia()
    setActivePinia(pinia)
    const instance = await createI18n()
    ...
    const expiresAtInput = dateInputs[0].element as HTMLInputElement

    const typeButton = wrapper.find('button[aria-label="Type"]')
    ...
    expect(wrapper.text()).toContain(new Date(card.received_at).toLocaleDateString())
  ...
  it('keeps metadata editing available after result fields are fixed', async () => {
    ...
    const typeButton = wrapper.find<HTMLButtonElement>('button[aria-label="Type"]')
  ...
  it('allows editing only expiration for automatic cards on the fighter page', async () => {
    ...
    const expiresAtInput = wrapper.findAll<HTMLInputElement>('input[type="date"]')[0]
  ...
  it('marks an inactive automatic red card for activation before saving on the fighter page', async () => {
    const pinia = createPinia()
    setActivePinia(pinia)
    const instance = await createI18n()
    const automaticCard: DisciplinaryCard = {
    ...
    await wrapper.find('[data-slot="checkbox"]').trigger('click')
    await wrapper.find('button[title="Edit"]').trigger('click')
    await wrapper.find('button[title="Make active"]').trigger('click')
