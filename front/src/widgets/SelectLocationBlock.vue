...
import SelectComponent from '@/widgets/SelectComponent.vue'
...
const onAddCountry = () => {
  ...
  const performAdd = async (name: string) => {
    await commonDataStore.addCountry(name)
    countries.value = await commonDataStore.fetchCountries()
  }
...
const onAddCity = () => {
  ...
  const performAdd = async (name: string) => {
    ...
    await commonDataStore.addCity(country_id, name)
...
const onAddClub = () => {
  ...
  const performAdd = async (name: string) => {
    ...
    await commonDataStore.addClub(city_id, name)
